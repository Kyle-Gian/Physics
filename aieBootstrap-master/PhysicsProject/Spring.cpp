...
Spring::Spring(RigidBody* a_actor1, RigidBody* a_actor2, float a_damping, float a_springCoefficient, float a_restLength, glm::vec2 a_contact1, glm::vec2 a_contact2)
    : PhysicsObject(JOINT), m_actor1(a_actor1), m_actor2(a_actor2), m_damping(a_damping), m_springCoefficeient(a_springCoefficient), m_restLength(a_restLength),
...
void Spring::FixedUpdate(glm::vec2 a_gravity, float a_timeStep)
{
	...
	//F = -kX - bv
	glm::vec2 force = direction * m_springCoefficeient * (m_restLength - length) - m_damping * relativeVelocity;
